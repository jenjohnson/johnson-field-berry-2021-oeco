...
%
%  Description:
%   Example simulation of temperature response with forward model
%
%  Compatibile with:
%   MATLAB R2020b
%
...
cd(resultsdir);
outputname = strcat('Example3-Temperature-',pathway_opt);
mkdir(outputname);
cd(workdir);

% Specify environmental conditions
n = 100;                                   % Steps in vector
data.Tin = transpose(linspace(10,40,n));   % Leaf temperature, C 
data.Qin = repmat(1200,n,1);               % PAR, umol PPFD m-2 s-1  
