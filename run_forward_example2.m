...
%
%  Description:
%   Example simulation of carbon dioxide response with forward model
%
%  Compatibile with:
%   MATLAB R2020b
%
...
cd(resultsdir);
outputname = strcat('Example2-CO2-',pathway_opt);
mkdir(outputname);
cd(workdir);

% Specify environmental conditions
n = 100;                                   % Steps in vector
data.Cin = transpose(linspace(1,1000,n));   % Mesophyll CO2, ubar 
